...
class AppStore extends Component {
  state = {initaltab: tabsList[0].tabId, searchinput: ''}

  getfilteredapp = () => {
    const {initaltab} = this.state
    const filtered = appsList.filter(each => each.category === initaltab)
    return filtered
  }
  ...
  getdesired = tabId => this.setState({initaltab: tabId})

  render() {
    const {initaltab, searchinput} = this.state

    console.log(searchinput)
    const filteredapp = this.getfilteredapp()
    console.log(initaltab)
    console.log(filteredapp)
    const searchResult = filteredapp.filter(each =>
      each.appName.toLowerCase().includes(searchinput.toLowerCase()),
    )

    return (
      <div className="bg_container">
        <h1 className="main_heading">App Store</h1>
        <div className="search_container">
          ...
          <img
            className="image_input button_tab"
        ...
        <ul className="tabcontainer">
          {tabsList.map(eachtab => (
            <TabItem
              isActive={eachtab.tabId === initaltab}
              selecting={this.getdesired}
              tabitemslist={eachtab}
              key={eachtab.tabId}
            />
          ))}
        </ul>
        <ul className="app_container">
          {searchResult.map(eachappser => (
            <AppItem eachappitems={eachappser} key={eachappser.appId} />
          ))}
        </ul>
      </div>
    )
